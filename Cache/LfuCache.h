...
public:
    ...
    void put(K key, V val) {
        ...
        if(iter == m_.end()) {
            if(size_ >= capacity_) {
                Node<K,V>* temp = tails[minClick_]->pre;
                erase(temp);
                delete temp;
                size_--;
            }
    ...
    void clean() {
        while(!isStop_) {
            std::this_thread::sleep_for(std::chrono:: milliseconds (1000));
            {
                ...
                // 清理过期键值对
                for(auto& kv : m_) {
                    if(std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - kv.second->timestamp).count() > ttl_) {
                        erase(cur);
                        m.erase(cur->key);
                        delete cur;
                        size_--;
...
private:
    ...
    void add(Node<K, V>* node) {
        ...
        if(!heads.count(click)) {
            heads[click] = new Node<K, V>();
            tails[click] = new Node<K, V>();
            heads[click]->next = tails[click];
            tails[click]->pre = heads[click];
        }
        node->next = heads[click]->next;
        heads[click]->next->pre = node;
        node->pre = heads[click];
        heads[click]->next = node;
    }
